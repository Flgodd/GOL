...
func distributor(p Params, c distributorChannels) {
	...
	// sends initial data to the broker
	if qStatus.Status == false {
		data := stubs.Request{OgWorld: world, ImageHeight: p.ImageHeight, ImageWidth: p.ImageWidth, Threads: 16}
	...
	go func() {
		for {
			...
			case <-ticker.C:
				// every 2 seconds a call is made to the broker to receive current data
			...
			case command := <-c.keyPresses:
				switch command {
				case 's': // used to output PGM image of current state
					c.events <- StateChange{turns, Executing}
					OutputWorldImage(c, p, w, turns)
				case 'q': // used to quit client
				...
				case 'p': // used to pause
					...
					for {
						...
						switch command {
						case 'p': // used to resume computation
				...
				case 'k': // used to shut down all components
					c.events <- StateChange{turns, Quitting}
					OutputWorldImage(c, p, w, turns)
					c.ioCommand <- ioCheckIdle
					Idle := <-c.ioIdle
	...
	// Closes the channel to stop the SDL goroutine gracefully. Removing may cause deadlock.
	close(c.events)
}
